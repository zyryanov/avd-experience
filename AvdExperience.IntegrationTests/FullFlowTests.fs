...
let ``clean workday: connecting and active durations are exact`` () =
    let stats, _ = computeWithTrace None false cleanWorkdayEnd cleanWorkdayEvents
...
let ``drop and reconnect: exactly one issue`` () =
    let stats, _ = computeWithTrace None false issueReconnectEnd issueReconnectEvents
...
let ``lock unlock: zero issues`` () =
    let stats, _ = computeWithTrace None false lockUnlockEnd lockUnlockEvents
    stats.TotalIssueCount |> should equal 0
    stats.TotalIssue      |> should equal TimeSpan.Zero
...
let ``multi-day: ByDay has entry for each day`` () =
    let stats, _ = computeWithTrace None false (multiDayEnd()) (multiDayEvents())
...
let ``writeEventsCsv: produces header and one row per event`` () =
    let _, trace = computeWithTrace None false cleanWorkdayEnd cleanWorkdayEvents
